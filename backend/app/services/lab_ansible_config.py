"""Phase 10B lab inventory + connectivity pilot config validation.

Sanitized previews only — never expose private key contents or secrets.
Defaults remain safe (MOCK_MODE=true, REAL_ANSIBLE_ENABLED=false).
"""
...
def build_lab_config_preview(
    ...
    *,
    ...
    # Timeout bounds: Settings clamps on load; still report if somehow out of range.
    ...
    if settings.real_ansible_enabled:
        ...
        # Safe default posture messaging for operators.
    ...
    if connectivity_allowed:
        status = "ok"
        # Keep only non-blocking notes (usually none when ok).
        errors = []
    elif not settings.real_ansible_enabled:
        status = "blocked"
