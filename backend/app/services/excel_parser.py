"""Chunked Excel parsing with openpyxl read_only mode.
...
def cell_to_str(value: Any) -> str | None:
    ...
    if isinstance(value, float):
        # Avoid scientific notation for IDs that arrive as numbers.
        if value.is_integer():
...
def validate_headers(headers: list[str]) -> dict[int, str]:
    """Validate required columns and return {column_index: field_name}."""
...
def iter_excel_rows(
    ...
    *,
    ...
    """Yield chunks of normalized rows using openpyxl read_only streaming."""
    path = Path(file_path)
    if not path.is_file():
    ...
    try:
        ...
        # Excel row numbers are 1-based; header is row 1, first data row is 2.
        excel_row_number = 1
